...
# 得到邮件的原始内容
# 这个过程主要负责从MDA到MUA下载并使用Parse粗略解析
def getMsg():
    # 准备相关的信息
    email = "1286542042@qq.como"
    # 邮箱的授权码
    pwd = "szribwlnphnfhgjg"
...
def parseMsg(msg, indent = 0):
    ...
    :param msg:
    :param indent: 描述邮件里面有几个邮件xxx类型的内容，展示的时候进行相应缩进
    :return:
    '''
    # 想办法提取出头部信息
    # 只有第一层的邮件中才会有相关内容
    ...
    if (msg.is_multipart()):
        ...
        # get_content_type是系统提供的哈拿书，得到内容类型
        content_type = msg.get_content_type()
        # text/plain或者text/html是固定值
        if content_type == "text/plaub" or content_type == "text/html":
